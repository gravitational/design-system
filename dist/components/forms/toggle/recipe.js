...
//#region src/components/forms/toggle/recipe.ts
const toggleSlotRecipe = defineSlotRecipe({
	...
	base: {
		...
		control: {
			...
			_hover: { bg: "buttons.secondary.hover" },
			_active: { bg: "buttons.secondary.active" },
			_checked: {
				bg: "success.main",
				_hover: { bg: "success.hover" },
				_active: { bg: "success.active" }
			},
			_focusVisible: {
				outline: "1px solid",
			...
			_disabled: {
				...
				cursor: "default",
				_hover: { bg: "interactive.tonal.neutral.0" },
				_checked: { bg: "interactive.tonal.success.2" }
			}
		},
		thumb: {
			position: "absolute",
			...
			_checked: { transform: "translate(var(--toggle-thumb-translate), -50%)" },
			_disabled: {
				opacity: "0.36",
				boxShadow: "none"
			}
