...
//#region src/components/forms/datePicker/recipe.ts
...
const datePickerSlotRecipe = defineSlotRecipe({
	...
	base: {
		...
		tableCellTrigger: {
			...
			"[data-view=month] &, [data-view=year] &": { width: "calc(var(--table-cell-size) * 1.75)" },
			textStyle: "sm",
			color: "text.main",
			bg: "transparent",
			borderWidth: "2px",
			...
			borderRadius: "sm",
			cursor: "pointer",
			_hover: { bg: "interactive.tonal.neutral.2" },
			"&[data-today]": { color: "text.main" },
			"&[data-selected]": {
			...
			"&[data-outside-range]": { color: "text.muted" },
			"&[data-unavailable]": {
				color: "text.disabled",
				cursor: "not-allowed"
			}
