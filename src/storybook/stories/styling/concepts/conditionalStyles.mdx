...
```tsx
<Box bg="gray.100" _hover={{ bg: 'gray.200' }}>
  Hover me
</Box>
```
...
<Box
  bg="gray.100"
  css={{
    '@container (min-width: 600px)': { bg: 'gray.200' },
  }}
>
...
<Box
  ...
  _disabled={{ bg: 'gray.50', cursor: 'not-allowed', opacity: 0.6 }}
  tabIndex={0} // Make the box focusable
>
  Interact with me
</Box>
...
```tsx
<Box className="group" p="spacing.4" bg="gray.100">
  <Text
